...
def plot_rgb_che_gedi(tile_id, predict_input_dir, input_file_names, inference_results, test_label_dir, biome):
    """Plotting routine
    ...
    rgb_input = rio.open_rasterio(input_file).sel(band=[4, 3, 2]).transpose("y", "x", "band").to_numpy()
    rgb_input = rgb_input/rgb_input.max() # Normalization of RGB bands
    pred = output_inference.detach().cpu().numpy()
    gedi = rio.open_rasterio(label_file).to_numpy()[0]
    gedi[gedi==-1] = np.nan
    
    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(14, 5))
    ax[0].imshow(rgb_input)
    im = ax[1].imshow(pred, cmap='Greens')
    ax[1].imshow(gedi, interpolation='none')
    plt.colorbar(im, ax=ax.ravel().tolist(), label='Canopy height (meters)')
    ax[0].set_title('Input HLS image (RGB bands)')
    ax[1].set_title('Predicted CHE overlaid by GEDI points')
    plt.suptitle(f'Biome: {biome}')